				 ...
				 const struct adc_channel_cfg *channel_cfg)
{
	...
	ads1x1x_write_reg(dev, ADS1X1X_REG_HI_THRESH, 0x8000);
	ads1x1x_write_reg(dev, ADS1X1X_REG_LO_THRESH, 0x0000);
...
static int ads1x1x_adc_start_read(const struct device *dev, const struct adc_sequence *sequence)
{
	...
	data->nr_of_reads = sequence->buffer_size / sizeof(int16_t);
	printk("nr_of_reads %d\n", data->nr_of_reads);
...
static int ads1x1x_adc_perform_read(const struct device *dev)
{
	...
	for(size_t i=0; i<data->nr_of_reads; i++) {
		// only needed to wait for alrert rdy interrupt here
		while(!alrtrdy_flag) {
			k_sleep(K_USEC(100));
		}
		alrtrdy_flag = 0;
		...
		*/
		*(data->buffer+i) = buf / (1 << (16 - config->resolution));
	}
...
static struct gpio_callback alrtrdy_data;

void alrtrdy_callback(const struct device *dev, struct gpio_callback *cb, uint32_t pins) 
{
	alrtrdy_flag = 1;
}
...
static int ads1x1x_init(const struct device *dev)
{
	const struct ads1x1x_config *config = dev->config;
	struct ads1x1x_data *data = dev->data;
	const struct gpio_dt_spec alrtrdy = GPIO_DT_SPEC_GET(DT_NODELABEL(alrtrdy), gpios);
	if (!device_is_ready(alrtrdy.port)) {
		LOG_ERR("GPIO %s not ready", alrtrdy.port->name);
		return -ENODEV;
	}
	int ret = 0;
	ret = gpio_pin_configure_dt(&alrtrdy, GPIO_INPUT);
	if (ret) {
		LOG_ERR("gpio_pin_configure_dt failed!");
	}
	ret = gpio_pin_interrupt_configure_dt(&alrtrdy, GPIO_INT_EDGE_FALLING);
	if (ret) {
		LOG_ERR("gpio_pin_interrupt_configure_dt failed!");
	}
	gpio_init_callback(&alrtrdy_data, alrtrdy_callback, BIT(alrtrdy.pin));
	gpio_add_callback(alrtrdy.port, &alrtrdy_data);
